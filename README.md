...
**覆盖提问（ORD-50）**：防遗漏机制，**不产生任何 artifact**。AI 只在两个 pause point（苏格拉底轮后 / 就绪评估前）各问一次「把这个想法讲给完全不懂的人听，哪一段你自己讲不下去」，回答按普通散文进当轮文档。**禁止 AI 预生成「技术/商业/营销」式维度大纲**（EXP-15 六视角独立否决：给出类别会收窄探索广度而不减少产出量）；措辞须结构性（「我没在前面看到 X」）非语义性（「X 讨论得不够」）。问法本身待 **EXP-17** 验证，连续 2 次零新增即停用。
