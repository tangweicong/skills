...
| `generalPurpose` | 多步实现 | plan 任务 | **仍属 Execute**；proj-plan 只规划不调用 |
...
|--------|---------------|------|----------|----------|
| 3 | subagent:explore | 搜 proj-plan assets | 文件清单 | review §问题 |
```

## 纪律

- proj-plan **只规划** handoff，**不启动** sub-agent（INV-04；启动归 `proj-run` skill，见 ORD-17）
- sub-agent 产出若推翻 ORD → `change-log` + 回 proj-shape
