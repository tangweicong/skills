...
### 4. 意图(to-be)重建评估 → 分支判定 ⚠️ EXP-06 待验证

按 §「分支判据」评估 intent 可重建度 → 给**分支建议**（可 plan / 仅 audit）+ 理由。
...
> **provisional**：以下判据由 EXP-05 dogfood（intent 易重建的简单端）初定，**难例尚未压测**（EXP-06）。用前以 GATE-S 人审批兜底。

意图可重建度按 4 维评估，**任一为「低」→ 倾向审计分支**：

| 维度 | 高 → 可 plan | 低 → 仅 audit |
|------|-------------|--------------|
| 来源丰富度 | 测试/代码/README/issue 多源印证 | 仅零散代码，无测试无文档 |
| 来源新鲜度 | 近期活跃、与代码一致 | 陈旧、与代码漂移严重 |
| 未完成工作可识别度 | 明确（TODO/issue/缺失实现可定位）| 模糊，说不清还差什么 |
| 矛盾程度 | 无方向性冲突 | 文档↔代码↔测试互相矛盾 |
